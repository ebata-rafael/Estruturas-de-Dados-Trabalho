...
node *chamarRotacao(node *raiz){
    ...
    if (raiz->fb == -2) // rotações pra direita
    {
        if (raiz->esq->fb == -1){
...
node *inserirNaArvoreAvl(node **raiz, cadastro *pessoa, int *possoAtualizarFb)
{
    ...
    {
        (*raiz)->esq = inserirNaArvoreAvl(&(*raiz)->esq, pessoa, possoAtualizarFb);
    ...
    {
        (*raiz)->dir = inserirNaArvoreAvl(&(*raiz)->dir, pessoa, possoAtualizarFb);
...
int criarLista(node **cidadaos, char *nomeArquivo)
{
    ...
    {                                                     // Itera sobre cada cidadão no array
        ...
        if (p == NULL)
        {
            ...
            *cidadaos = inserirNaArvoreAvl(cidadaos, novaPessoa, &possoAtualizarFb);
        }
        else
        {
...
void relatorio(node *cidadaos, int idadeMinima, int idadeMaxima) {
    ...
    node **hash = criarEstruturaRelatorio();
...
void atualizaFb (node *raiz) {
    ...
    if (raiz->esq != NULL || raiz->dir != NULL)
    {
        if (raiz->esq == NULL)
        {
            raiz->fb = 1; // lado esquerdo é null
        }
        else if (raiz->dir == NULL)
        {
            raiz->fb = -1; // lado direito é null
        }
        else{

            raiz->fb = (abs(raiz->esq->fb) - abs(raiz->dir->fb));
        }
    }
    else{
        raiz->fb = 0; // os dois filhos sao null
    }
}
...
struct node* rotacaoDireita(struct node* raiz) {
    ...
    struct node* filhoEsquerdo = raiz->esq;
    struct node* neto = filhoEsquerdo->dir;
...
struct node* rotacaoEsquerda(struct node* raiz) {
    ...
    struct node* filhoDireito = raiz->dir;
    struct node* neto = filhoDireito->esq;
